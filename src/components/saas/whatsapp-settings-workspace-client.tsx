...
export function WhatsAppSettingsWorkspaceClient({
  initialValues,
  credentialsReady,
  credentialsSource,
  hasSavedSecrets,
  ...
  const webApiDashboardReady = Boolean(
    masAccountDashboard ||
      (masLinkStatus && provider === "messageautosender"),
  );
          ...
          {provider === "messageautosender" ? (
            <>
              <div className="ws-settings-wallet-head">
                <h2>Account</h2>
                <span
                  className={`ws-web-api-status-pill${masAccountDashboard?.connected ? " is-connected" : ""}`}
                >
                  {masAccountDashboard
                    ? channelStatusLabel(
              ...
              {webApiDashboardReady ? (
                <div className="ws-settings-wallet-stats">
                  <div className="ws-settings-wallet-stat">
                    ...
                    <strong>
                      {formatDashboardValue(masAccountDashboard?.accountType)}
                    </strong>
                  </div>
                  <div className="ws-settings-wallet-stat">
                    <span>Valid Until</span>
                    <strong>
                      {formatDashboardValue(masAccountDashboard?.validUntil)}
                    </strong>
                  </div>
                  <div className="ws-settings-wallet-stat">
                    <span>Credit Count</span>
                    <strong>
                      {formatDashboardValue(masAccountDashboard?.creditCount)}
                    </strong>
                  </div>
                </div>
              ) : (
                <p className="ws-settings-wallet-empty">
                  Save Web Based API credentials to view account stats.
                </p>
              )}
            </>
          ) : (
            <>
              ...
              {walletLoaded ? (
                ...
                <p className="ws-settings-wallet-empty">
                  Could not load wallet balances. Save a valid API key, then
                  refresh this page.
                </p>
              ) : (
                <p className="ws-settings-wallet-empty">
      ...
      >
        <div className="ws-settings-connection-head">
          ...
          <p className="ws-settings-connection-hint">
            Source: <strong>{credentialsSource}</strong> &middot; After saving, finish
