...
import {
  ...
  leadPaymentApplied,
  type LeadPaymentSplit,
} from "@/lib/leads/payment-totals";
...
function groupProjectsByLead(
  ...
  paymentByLead: Map<string, LeadPaymentSplit>,
  sectionKey: string,
  ...
  for (const order of rows) {
    ...
    const split = paymentByLead.get(order.lead.id);
    const received = split?.collected ?? 0;
    const adjusted = split?.adjusted ?? 0;
    const due = Math.max(0, Math.round((value - leadPaymentApplied(split)) * 100) / 100);
    const existing = byLead.get(order.lead.id);
    if (existing) {
      existing.orders.push(order);
      existing.value += value;
      existing.due = Math.max(
        0,
        Math.round((existing.value - existing.received - existing.adjusted) * 100) / 100,
      );
    } else {
  ...
  return [...byLead.values()].map((entry) => {
    ...
    return {
      ...
      meta:
        entry.received > 0 || entry.adjusted > 0
          ? `collected ${formatInr(entry.received)}${
...
export default async function CrmProjectsPage({
  ...
  const receivedOnProjects = orders.reduce((sum, order) => {
    return sum + (paymentByLead.get(order.lead.id)?.collected ?? 0);
  }, 0);
  const overdueValue = [...new Set(orders.map((order) => order.lead.id))]
    ...
    .reduce((sum, leadId) => {
      ...
      return sum + Math.max(0, leadValue - leadPaymentApplied(paymentByLead.get(leadId)));
    }, 0);
