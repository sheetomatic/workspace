...
import { normalizeChecklistImportRow } from "@/lib/checklists/template-import";
...
export async function POST(request: Request) {
  ...
  if (
    ...
    !canConfigureChecklists(user)
  ) {
    return NextResponse.json(
      { error: "Manager access to PC setup is required to import templates." },
      { status: 403 },
    );
  }
  ...
  const parsed = rawRows.map((raw, index) =>
    normalizeChecklistImportRow(raw, index + 2),
  );
