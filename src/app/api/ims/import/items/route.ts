...
import { normalizeItemImportRow } from "@/lib/ims/item-import";
...
export async function POST(request: Request) {
  ...
  if (!hasWorkspaceModule(user, "IMS") || !hasMinimumRole(user.role, "MANAGER")) {
    return NextResponse.json(
      { error: "Manager access to IMS is required to import items." },
      { status: 403 },
    );
  }
  ...
  const parsed = rawRows.map((raw, index) => normalizeItemImportRow(raw, index + 2));
